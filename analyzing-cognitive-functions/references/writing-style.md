...
- **R6** 分析层词汇零容忍：正文不出现「梯队 / 断层 / 分数分层 / inflation」等，用白话（「最前面几项几乎不分先后」「八项普遍偏低」「后面断崖式掉下去」）。形态学名不在此列（尖塔型 / 双峰型 / 高原型 / 塌陷型 / 反常型 可在 01 章尾「你的分布形状：」小节形态行使用）。
...
- 允许整体形态的白话描述（服务于 01 章尾「你的分布形状：」小节），如「最前面的三项挤在一起，后面断崖式掉下去」。
