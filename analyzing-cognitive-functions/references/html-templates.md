...
## 2. 单人报告结构（hero + 01–07 + footer，章节恰 7 个）

**hero**：`.kicker`「Jungian Cognitive Functions · 人格坐标报告」+ 定位句 h1 + `.lede` 固定句 + 雷达（雷达下不设图注段）。

**01 · 类型与八维栈**（承载：类型与八维栈）
- 柱状图**先行**（精确分数，0–100 标尺，占主视觉）。
- 八维栈明细表（`table.lt`）：功能 · 分数 · 栈位 · 原型白话 · 对位（✓/≈/✗）。**每个维度恰一行**；行序按分数降序，不设排名列。
- 类型判定块（紧凑：栈序一行 + 对位摘要）：
...
**02 · 形状与分化**（承载：形状与功能分化）
- 形状用**一句话**带过（白话，禁形态学名）。
- **重点是结论**：分化判词加粗/高亮放最前（主导能独立工作、不被其他功能干扰 = 分化好；分数糊成一团 = 分化差），再补 3 分噪声带的名次口径与「对你意味着什么」。
...
**04 · 回血与盲区**（承载：回血场合 + 压力退行 + 偏离 B）
- 回血场合表先行：「什么场合让你回血」（每行 = 场景 + 哪个功能在回血）。
- 盲区小节标题照录「你的盲区：并非不足，是成本」。
