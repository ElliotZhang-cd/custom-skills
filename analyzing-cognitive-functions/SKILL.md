...
- **Phase 2 — 生成报告**
  单人结构：hero + 01–07 章 + 附录 + footer（骨架见 `references/html-templates.md`）。唯一视觉事实源 = `examples/mbti_sample.html`。固定文本块**照录，不得改写**。双人用 JS 数据驱动版，**不判合分**。命名：`mbti_<用户代号>.html` / `mbti_<A>_<B>.html`。
...
**能得出**（报告 01–07 章 + 附录承载）：类型与八维栈、形状与分化（01 章）/ 压力下的退行形态与偏离（02 章）/ 能量回血与耗竭（03 章）/ 四条轴能量偏向（04 章）/ 人际投射与冲突机制（05 章）/ 个体化路径（06 章，最低那格 = 成长空间最大，它是通往自性的门）。
