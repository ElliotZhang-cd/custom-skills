...
│   ├── interpretation-library.md   # 分数模式→文案素材库（含扁平剖面策略；初稿，待咨询师终审）
...
- **v3.1（2026-09-12，当前）**：流程精简——双人规范整体下沉 `couple-template.md`（SKILL.md 只留分流）；输入契约改为"平台固定输出的文本或 JSON"（skill 不感知平台）；问询清单精简为代号+常模两项（人数默认单人，两份数据或明示触发双人）；重复规则收敛到唯一规范位；浏览器检查改为条件触发（新增 `scripts/run_regression.py` 基线回归命令）；内容层新增数量弹性规则与扁平剖面文案策略（library §6）、成长动作库扩容至 16 条、「原理」字段限定库内原理句；分数引用改为「子维度 · 档位」制（禁裸原始分），lint 同步升级。
