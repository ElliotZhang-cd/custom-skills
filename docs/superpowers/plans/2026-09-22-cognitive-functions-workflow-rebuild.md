...
- **用户审阅修订（第二轮，样例审阅意见）**：
  15. 01 章柱状图先行 + 八维栈五列紧凑表（每维恰一行；锁不住并列时分情况开两个列表，不挤一张表）+ 紧凑判定块；02 章形状一句话带过、结论加粗高亮前置；删除附录（内容并入 01 栈明细表）；「什么场合让你回血」与盲区合并为 04 章（「你的盲区：并非不足，是成本」降为盲区小节标题）；删 hero 雷达图注段。

## 用户审阅修订（第三轮）——实施计划（2026-09-22）

反馈要点：柱状图矮胖要竖瘦；柱下注解删除（其"常用/费力"解读不准——正确解读：分数高=意识里发展得好，低分=藏进无意识/被投射，两处限定：inflation 例外、低分≠没能力）；八位置白话移附录；附录增八维投射（用户《八维投射.txt》整合）；02 改「你的分布形状：」+ 1.形态 / 2.结论 结构（形态学名解禁）；04 改「心理能量的分布与管理方式——哪类活动回血、哪类活动耗竭」；新增动力学章（基于荣格动力学分析八维位置，重点偏离）；补足"压力下的退行形态"与"人际冲突的机制"两个此前缺位的结论。章节变为 **01–08 + 附录**。执行顺序：子代理 B（规格与工具同步）→ 子代理 A（样例改版）→ lint 双 PASS 终验。

### B. 规格与工具同步（先行）
B1 `references/html-templates.md`：结构改「hero + 01–08 + 附录 + footer，章节恰 8 个（附录另计）」；01（柱状图先行 + 4 列栈表：功能·分数·栈位·对位，白话列移附录 A + 紧凑判定块，两案分列表规则不变）；02（h2「你的分布形状：」；结构=1. 形态：点名五类之一（尖塔/双峰/高原/塌陷/反常）+一句定义；2. 结论：两句式（肯定句+"可能…"保留句）。五形态结论模板：尖塔型→主导功能分化清晰，自我认同稳固。可能认知工具箱比较单一，适应面可能不够宽。/ 双峰型→不同轴=主辅齐飞理想；同功能反态度=态度未分化、身份困惑常见（两解+自查）/ 高原型→未分化 vs 社会化良好但自我认同稀薄（两解+自查）/ 塌陷型→优先怀疑答题防御或耗竭，建议重测、推断降权 / 反常型→转动力学章偏离读法）；03 不变；04（04 · ENERGY MANAGEMENT「心理能量的分布与管理方式」，lead 点题「哪类活动让你回血，哪类活动让你耗竭」= 回血表 + 耗竭表（每行场景+耗竭的点）+ 管理方式小结（回血进日程、耗竭工具化/外包/限时、补电顺序））；05（05 · POSITION DYNAMICS「基于荣格动力学的八维位置分析」= ①正常趋势 1>2>3>4≳5≈6>7>8 与实测对照 ②偏离重点：偏离 A/B 两解读法（就地自查）③压力下的退行形态（固定小节名：第 8 位出口=退行信号+护栏；第 4 位膨胀=另一种退行形态）④第 6 位批判者音量 ⑤h3 固定句「你的盲区：并非不足，是成本」+省力做法表）；06（「人际与投射：人际冲突的机制」：投射机制=功能定投什么、态度定形态（内倾→无人称判决「这才是…」；外倾→条件句「有你在，我才能…」，听语法辨方向）+「最容易点燃你情绪的=对方带着你未被拥有的部分」+ 指向附录 B）；07 个体化（自查清单留章尾）；08 边界（第 4 条改「清单见 07 章尾」）；附录规格 A/B（见 A8 内容）；柱状图几何（见 A1）且不设 chart-note。
B2 `references/writing-style.md`：形态学名解禁（尖塔型/双峰型/高原型/塌陷型/反常型可在 02 章形态行使用）；§5 词表删形态学名行；§7「04 章盲区小节标题」→「05 章盲区小节标题」；§7 边界第 4 条「（清单见 06 章尾）」→「（清单见 07 章尾）」。
B3 `scripts/lint_report.py`：FORBIDDEN_WORDS 删 5 个形态学名；SINGLE_REQUIRED 增「压力下的退行形态」与「附录」；单人章节检查 s1–s8。
B4 `SKILL.md`：Phase 2「01–08 章 + 附录」；结论口径映射行同步。
B5 `evals/evals.json`：用例 1「七章齐全」→「01–08 章 + 附录齐全」；用例 3（高原型）改按 02 章 1.形态/2.结论 结构断言。
B6 验证 + commit。

### A. 样例改版（examples/mbti_sample.html）
A1 柱状图重绘（竖瘦）：viewBox `0 0 680 380`；基线 y=320；网格线 y=320/264/208/152/96/40（标 20–100，标签 x=22）；柱宽 38，8 槽位（Ni,Fe,Ti,Se,Ne,Fi,Te,Si）x=51,131,212,292,372,452,533,613；柱高=分数×2.8（218.4/201.6/179.2/162.4/131.6/114.8/92.4/72.8），柱 y=101.6/118.4/140.8/157.6/188.4/205.2/227.6/247.2；数值标签 y=柱顶−9（92.6/109.4/131.8/148.6/179.4/196.2/218.6/238.2）；功能名 y=348；横中心 x=70/150/231/311/391/471/552/632。
A2 删柱下 chart-note 整段（正确解读进附录 B 引言）。
A3 01 栈表改 4 列（功能·分数·栈位·对位；栈位=「1 · 英雄」式）。
A4 02 重写：h2「你的分布形状：」；「1. 形态：**尖塔型**（一个极高，其余平滑递减）」「2. 结论：主导功能分化清晰，自我认同稳固。可能认知工具箱比较单一，适应面可能不够宽。」
A5 04 重写为能量管理章：回血表（现成）+ 耗竭表（琐碎流程与日程维护 / 现场应变与即时表现 / 立场攻防式争论 / 即兴发散出方案，各配耗竭的点）+ 管理方式（回血进日程、耗竭限时或工具化/外包、连续耗竭后的补电顺序）。
A6 新增 05 动力学章（内容按 B1-05 规格，INFJ 数据：正常趋势吻合；偏离 B=第 4 位 Se 58 不低两解+自查；接近偏离 A=Ti 64 与 Fe 72 差 8 分边缘解读；退行形态=Si 26 出口+护栏；第 6 位 Fi 41 音量不大借 Ti 外壳；盲区固定句+三行省力做法）。
A7 章序调整：原 05→06（章名人际与投射：人际冲突的机制，投射机制按 B1-06 升级并引附录 B）；原 06→07；原 07→08（边界第 4 条「06 章尾」→「07 章尾」）。
A8 新增附录（`<section id="appendix">`，08 章后 footer 前）：
- **附录 A · 八个位置的名字（白话）**：1 英雄=你最自然、最拿手、最认同自己的部分；2 好父母=你照顾、支撑别人的那双手；3 永恒少年=放松时才玩得起来、孩子气、也容易在这里受伤的部分；4 阿尼玛/阿尼姆斯=最费力、最想回避，也是最可能带你长大的门；5 对立人格=被人反对时冒出来的防御姿态；6 批判父母=内心那个挑剔的声音；7 欺骗者=压力下自以为聪明、容易把你带沟里的把戏；8 恶魔=压力最大时才出现、破坏性最强的出口。
- **附录 B · 八维投射**：引言（分数低的部分藏在无意识里，最容易以投射的形态出现；功能决定投什么，态度决定投什么形态——内倾→无人称判决「这才是…」，外倾→条件句「有你在，我才能…」；听语法就能分辨方向）+ 8 条列表（格式：**功能** 投的内容｜句型与例句｜对象被赋予｜失效方式）：
  ...
  + 四领域小表（思考=真：可信/懂/能办成；情感=值：对/属于；感觉=实：真的存在/真的在场；直觉=义：意味着什么/可能成为什么；同一对的两个功能是同一领域的收敛版与发散版，最容易互相冒充）+ 层级说明（本清单是灵魂意象层——投的是未活出的一个部分；自性投射是另一层级，投的是整全本身；Ni 与 Fi 最容易从「部分」升级成「整全」，其他六个要升级得借道这两个）。
A9 lint 至双 PASS + commit。

## 用户审阅修订（第五轮）——实施计划（2026-09-23）

原则：**正文只给结论**——不解释理论、不写"见 XX 章/参考"式指引；解释性内容仅允许存在于附录。章节回为 **01–07 + 附录 + footer（章节恰 7 个）**。

- **01 · TYPE & STACK**：柱状图 → 类型判定（推测的类型）→ 八维栈表（4 列）→ 章尾「你的分布形状：」小节（原 02 整体并入）：「1. 形态：尖塔型」（只写名称，删定义括注）+「2. 结论：主导功能分化清晰，自我认同稳固。可能认知工具箱比较单一，适应面可能不够宽。」→ 固定句「别把任何标签当身份证」（保留，删其后的"真正有信息量的是…"指引句）。
- **02 · POSITION DYNAMICS**「基于荣格动力学的八维位置分析」：删理论开头与正常趋势说明，直接两块——**偏离**（偏离 B：第 4 位 Se 58 两解+自查；接近偏离 A：Ti 64 vs Fe 72 差 8 分边缘读法）与 **压力下的退行形态**（固定小节名照录；Si 26 出口+护栏、第 4 位膨胀=另一种退行形态）；盲区固定句小节保留但第 4/6/8 位各压成一行结论（省力做法列移 06）。
- **03 · ENERGY MANAGEMENT**「心理能量的分布与管理方式」：首行注明**偏内倾/偏外倾**（按主导态度定 + 一句补偿说明；样例=偏内倾）→ 回血场景列表（单列，删"回血的点"列）→ 耗竭场景列表（单列）。无管理方式小节。
- **04 · 四条轴**（原 03 整体后移至此）：减理论重结论——删章引言理论段与 4 个场景块；每轴 = 天平图 + 一句话结论（caption 三档句）+ 两端对照表（结论列保留）；双弱 pc-t 表保留；主辅协作压成一句结论。
- **05 · PEOPLE & PROJECTION**「人际与投射：人际冲突的机制」：删投射理论段与「见附录 B」指引；保留固定句 lead、人际冲突机制结论 1–2 句（最容易点燃你情绪的=对方身上带着你未被拥有的部分）、relg 四键、兼容地图。
- **06 · INDIVIDUATION**「个体化路径」：吸收管理方式（回血进日程、耗竭工具化/外包/限时、补电顺序）+ 原盲区省力做法 + 发展建议表 + 章尾自查清单。
- **07 · BOUNDARIES**：第 4 条括注「（清单见 07 章尾）」→「（清单见 06 章尾）」。附录 A/B 不动。

执行：子代理 B（规格同步：html-templates §2 重写、writing-style 章号同步、SKILL.md Phase 2 与结论口径映射、lint 单人章节 s1–s7、evals 章号断言）→ 子代理 A（样例按上重排精简）→ 终验（lint 双 PASS；样例正文 grep「见附录|见 0[0-9] 章|真正有信息量|场景块」=0）。
